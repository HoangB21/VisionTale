...
class ImageService(SingletonService):
    ...
    def _connect_websocket(self) -> websocket.WebSocketApp:
        ...
        if self._ws_error:
            raise Exception(
                f"Failed to connect to WebSocket: {self._ws_error}")
    ...
    def _wait_for_execution(self, prompt_id: str, timeout: int = 300) -> Tuple[bool, Optional[Dict]]:
        """Wait until image generation is completed."""
        start_time = time.time()
        ...
        print(f"Execution timeout: {prompt_id}")
        return False, None
    ...
    def generate_images(
        ...
        if output_dirs and len(output_dirs) != len(prompts):
            raise ValueError("The length of output_dirs must match prompts")
            ...
            def generate_worker():
                try:
                    for i, (prompt, output_dir) in enumerate(zip(prompts, output_dirs)):
                        ...
                        # Use cloned workflow data to avoid modifying the original
                        current_workflow = json.loads(
                            json.dumps(workflow_data))

                        current_workflow = self.workflow_service.update_workflow_prompt(
                            current_workflow, prompt, current_params.get('negative_prompt', ''))
                        current_workflow = self.workflow_service.update_workflow_seed(
                            current_workflow, current_params['seed'])
                        current_workflow = self.workflow_service.update_workflow_params(
                            current_workflow, current_params)
                        if self.config['comfyui'].get('reference_image_mode', True) and current_params.get('reference_image_paths'):
                            ref_paths = current_params.get(
                                'reference_image_paths')
                            if i < len(ref_paths):
                                current_workflow = self.workflow_service.update_workflow_reference_image(
                                    current_workflow, ref_paths[i])

                        self._connect_websocket()

                        try:
                            prompt_id = self._send_workflow(current_workflow)
                            success, history = self._wait_for_execution(
                                prompt_id)
                    ...
                    logger.error(
                        f"Error in generation worker for task {task_id}: {str(e)}")

                finally:
                    self.tasks[task_id]['current_prompt'] = None
                    logger.info(
                        f"Task {task_id} finished with status: {self.tasks[task_id]['status']}")
    ...
    def _save_image(self, image_data: Dict[str, Any], output_dir: str):
        """Save generated image to the specified directory."""
        try:
    ...
    def get_workflow(self, name: str) -> Optional[Dict[str, Any]]:
        """Get detailed information of a specified workflow."""
